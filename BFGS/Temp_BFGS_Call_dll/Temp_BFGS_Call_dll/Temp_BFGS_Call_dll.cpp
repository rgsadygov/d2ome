...
int main(array<System::String ^> ^args)
{
   // Console::WriteLine(L"Hello World");
	...
	int i = 0, j = 0, k = 0, l = 0, nret, i1=0, ntimepoints=7;
	int imodel;
	int count=0;
	FILE *fp, *fp2;
	...
	{
		fTime[i1] = fTime[i1]/tmax;///24;///tmax;//24;
	}
	

	imodel=4;

	LBFGS ^ lbfgs = gcnew LBFGS(fTime, ntimepoints, imodel, "Two_Compartment_exponential");

    //LBFGS ^ lbfgs = gcnew LBFGS(fTime, ntimepoints, imodel, "One_Compartment");

	//fp = fopen("NASH_median_NL_Test_all.csv","r");

	fp = fopen("T2D3_PosMon_NL.csv","r");

	//fp = fopen("NASH_median.csv","r");
 ...
 */
	...
	*/
	...
	//fp2 = fopen("Rate_2CExpo_Brain.csv","w");

	//fp2 = fopen("Rate_1C_NASH_median_Test.csv","w");

	//fp2 = fopen("Rate_2CExpo_NASH_median_Test_all.csv","w");

	fp2 = fopen("Rate_2CExpo_T2D3_PosMon_NL_bothnorm_day.csv","w");

	//fp2 = fopen("Rate_2CExpo_Liver_Test.csv","w");
		//fp2 = fopen("Rate_2CExpo_NASH_median.csv","w");
			//fp2 = fopen("Rate_2CExpo_NASH_median_tnorm.csv","w");
		//fp2 = fopen("Rate_2Compartexpo_Liver_nonorm_median.csv", "w");
		//fp2 = fopen("Rate_2Compartexpo_Brain_nonorm.csv", "w");
		//fp2 = fopen("APOB_HUMAN.Quant_NL_both_norm_3.csv", "w");
    //fp2 = fopen("Rate_2Compart_Brain_both_norm_rand_100.csv", "w");
	//fp2 = fopen("test_expo.csv", "w");
	...
	while(fgets(szLine, sizeof(szLine), fp) != NULL)
	{
		...
		*/
		...
		{
			//1 compartment
			fprintf(fp2, "%15.6f,  %d, %f\n", lbfgs->fParams[0]/tmax, nret,fx1);
		}
		else if(2==imodel)
		{
			//2 compartment
			fprintf(fp2, "%15.6f,   %15.6f,  %d, %f, %f\n", kdfinal, ksfinal, nret, rkd, fx1);
		}
		else if(3==imodel)
		{
			//1 compartment exponential
			fprintf(fp2, "%15.6f,  %d\n", (exp(lbfgs->fParams[0])), nret,fx1);
		}
		else if(4==imodel)
		{
			//2 compartment exponential
			fprintf(fp2, "%15.6f,%15.6f,  %d,%15.6f\n", (exp(kdfinal))/tmax,(exp(ksfinal))/tmax, nret,fx1);
		}
