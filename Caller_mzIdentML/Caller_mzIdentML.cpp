...
//Added by JA 2016.08.08 AllExperiments added as input arg
//Added by JA 2016.08.14 ProteinPeptides added as input arg
...
typedef vector <charArray> CharArray;

bool bCClinic = true;    // a special pipeline developed for Cleveland Clinic collaborators, make it true before sending to ClevelandClinic
...
int nProteinConsistency = 1;   //number of experiments a protein needs to be observed in, for quantification
int nPeptideConsistency = 1;
float ElutionTimeWindow = 5.0;  // the time (min) 
double dMassAccuracy = 0.05;
...
/*
*Checks each position and returns true/false based on match/mismatch
Added by MR 11/16/2016
*/
bool check_matching_modlocations(List<int> ^m1, List<int> ^m2) //Checks each position and returns true/false based on match/mismatch
{
...
/*
*Find and add the missing time points of a peptide
Added by MR 11/15/2016
*/
...
void WriteResults(ExperimentCollection ^allResults, array<System::String ^> ^args)
{
	...
	if(filename4 != "")
		//tempFile = filename4 + "\\" + allResults->ExperimentsList[i]->sExperimentFile->Substring(0, j) + "Proteins.csv";
		tempFile = filename4 + "\\"+ "Proteins.csv";//change on 05/13/2016
	else
		//tempFile = allResults->ExperimentsList[i]->sExperimentFile->Substring(0, j) + "Proteins.csv";
	...
	for(k=0; k < allAccessions->Count; k++)
	{
		...
		for(i=0; i < 30 && i < allProteins[k]->Length; i++)
		{
		if(allProteins[k][i] == ',')
		{
		szTemp[i] = ' ';
	...
	//	for(j = 0; j < allResults->ExperimentsList[i]->ProteinsList->Count; j++) // over proteins of a single experiment
	//	{
	//		//if(allResults->ExperimentsList[i]->ProteinsList[j]->description->IndexOf("Hemoglo") != -1)
	//		{
	//			printf("Acession: %s\t%-25.23s SC %2d %5.f %5.1f %2d %s %d\n", 
	//				allResults->ExperimentsList[i]->ProteinsList[j]->accession,
	//				allResults->ExperimentsList[i]->ProteinsList[j]->description,
	//				allResults->ExperimentsList[i]->ProteinsList[j]->nSpectralCount,
	//				allResults->ExperimentsList[i]->ProteinsList[j]->ProteinScore,
	//				allResults->ExperimentsList[i]->ProteinsList[j]->SeqCoverage,
	//				allResults->ExperimentsList[i]->ProteinsList[j]->nDistinctSequences,
	//				allResults->ExperimentsList[i]->sExperimentFile,
	//				allResults->ExperimentsList[i]->ProteinsList[j]->Peptides->Count);

	//			for(int k=0; k < allResults->ExperimentsList[i]->ProteinsList[j]->Peptides->Count; k++)
	//			{
	//				 printf("[%d]%s %10.5f Scan = %d\t", k, allResults->ExperimentsList[i]->ProteinsList[j]->Peptides[k]->Peptide,
...
/*
* A method to write results for Cleveland Clinic
...
void D2O_H2O(ExperimentCollection ^allResults,array<System::String ^> ^args)
{
	...
	*/
	...
	//ConsistentPeptides(allResults, dIonScoreCut, dExpectCut, allResults->ExperimentsList->Count -1
...
int Start_Process(array<System::String ^> ^args)
{
	...
	if(bSpectra)
	{
		...
		ProteinRates(ExperimentTime, BodyWaterEnrich);
	}
...
int ReadAbundances(array <String ^> ^smzML, ExperimentCollection ^AllExperiments)
{
	...
	*    higher than 20.
	*/
	FILE *fpLog = fopen("HeavyWater.Log", "w");

	//Added by JA 2016.09.01
	//FILE *ftemp = fopen("IntermediateResults.csv", "w");

//	fprintf(ftemp,"Peptide,Charge,M/Z,Raw File,Scan,Half Width,Left Width,Right Width,Retention Time Peak\n");
	if(fpLog == NULL)
	{
		printf("Cannot write in this folder - will create a problem\n");
	...
	{	
		...
		/*MzML ^mzml = gcnew MzML(AllExperiments->ExperimentsList[i]->smzML);

		mzml->ReadOffset();*/

		//0.05 is the mass accuracy. Eventually set it to read from the file, 15 minutes is the elution time window from the elution time
		//		DetectAndIntegratePeak ^DetectPeaks = gcnew  DetectAndIntegratePeak(AllExperiments->ExperimentsList[i]->smzML, 0.005, ElutionTimeWindow);

		DetectAndIntegratePeak ^DetectPeaks = gcnew  DetectAndIntegratePeak(AllExperiments->ExperimentsList[i]->smzML, 0.045, ElutionTimeWindow);
		//0.045
		...
		for(j = 0; j < AllExperiments->ExperimentsList[i]->ProteinsList->Count; j++) // iterate over proteins of a single experiment
		{
			if(AllExperiments->ExperimentsList[i]->ProteinsList[j]->ProteinScore > ProteinScoreCut)
			{
				...
				{
					//if(AllExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[k]->bQuant)   //only those peptides that are used for quantification
					Prev_SpecMass = 0;
					...
					{					
						...
						Prev_SpecMass = dSpecMass; //added by MR 11/18/2016
						...
						//ReadIsotopes(nScan -1, AllExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[k]->SeqMass,
						//ReadIsotopes(nScan -1, dMassToCharge, nCharge, mzml,

						//DetectPeaks->ReadIsotopes(nScan -1, dMassToCharge, nCharge,
						//AllExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[k]->dIsotopes);
						/*
						sets the candidate start and end elution times to the first and last time that a peptide
						has been identified in MS2. If a peptide was identified only once,
						then the candidate start and end elution times are equal to the MS2 identification.
						*/

						//Debug code added by JA 2016.09.02
						...
						//Debug code for problem peptide
						//if(AllExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[k]->Peptide == "QNcELFEQLGEYK") //"ccTESLVNR")
						//{
						...
						/*
						if("YGAVTFSR" == AllExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[k]->Peptide && i==AllExperiments->ExperimentsList->Count-1)
						{
						printf("Problem Peptide");
						}
						*/
						...
						*/
						//printf("\nBefore DetectPeaks: Ret time = %f, Spec Mass = %f, Scan = %d, Start Elution = %f, End Elution = %f\n",fRetTime,dMassToCharge,nScan,StartElution/60.0,EndElution/60.0);
						...
						fprintf(ftemp,"%s,",AllExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[k]->Peptide);
						fprintf(ftemp,"%d,",AllExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[k]->nCharge);
						fprintf(ftemp,"%f,",AllExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[k]->SeqMass);
						fprintf(ftemp,"%s,",sTemp);
						fprintf(ftemp,"%d,",nScan-1);
						fprintf(ftemp,"%f,",w[0]);
						fprintf(ftemp,"%f,",w[1]);
						fprintf(ftemp,"%f,",w[2]);
						fprintf(ftemp,"%f\n",w[3]);
					...
					else if(!AllExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[k]->bPeptidePassed && i-1 >= 0 /*&& AllExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[k]->Peptide=="ALVDTLK"*/) 
					{
						...
						if(!peptideAssigned)   // remove this block
						{
							...
							/*else
							{
								printf("CCCCCCCCCCCCCCCCCCCCCCCCCCCCould not find the time point information\n");

								printf("Could not DOOO Assigning time points: Protein :%s, Peptide : %s, RetTime : %f, SpecMass :%f, SeqMass :%f\n",protein, peptide, fRetTime/60.0, SpecMass, SeqMass);
							
								AllExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[k]->bPeptideTimePoint = false;
							}*/
							//}//if(i-1>=0)
	...
	float ProteinScoreThreshold, int nExperimentThreshold)
{
	...
	*/
	...
	for(k = 0; k < lProteins->Count; k++)
	{
		if(lProteins[k]->ProteinInExperiments->Count >= nExperimentThreshold)
		{
	...
	double dExpectationScoreThreshold, int nExperimentThreshold)
{
	...
	for(k=0; k < lPeptides->Count; k++)
	{
		...
		{
			/*printf("Checking peptide %s charge %d, nMods = %d, NumerExpe = %d\n", lPeptides[k]->Peptide->Peptide, 
			lPeptides[k]->Peptide->nCharge, lPeptides[k]->Peptide->ModLocations->Count,
			lPeptides[k]->PeptideInExperiments->Count);*/
			//count_prot=0;
			...
			for(i=0; i < allExperiments->ExperimentsList->Count; i++) //over experiments
			{
				...
				//Added by MR 11/21/2016
				//if(!aProtein)
				//	 aProtein = gcnew ProteinSet(); //Allocating memory 
				//if(!aPeptide)
				//	   aPeptide = gcnew PeptideHolder();				
				exp_no = i;
				...
				for(j = 0; j < allExperiments->ExperimentsList[i]->ProteinsList->Count; j++)  //over all proteins of an experiment
				{
					...
					for(l = 0; l < allExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides->Count; l++)
					{
						...
						{	                                                                                                               // as lPeptides  lPeptides[k]->bPeptidePassed - 
							//bFoundProtein = true;																						   // cosidering only passed peptides
							...
							/*if(0 == i)
							{
								printf("experime = %d, Protein = %s, peptide %s  %d Score %10.5f  %10.5f %10.5f %d\n", i, allExperiments->ExperimentsList[i]->ProteinsList[j]->accession, 
									...
									allExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[l]->nScan, allExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[l]->dIonscore,
									allExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[l]->fFirstID_Elution/60, 
									allExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[l]->fLastID_Elution/60,
	...
	for(l = 0; l < allExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides->Count; l++)
	{
	if(allExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[l]->Peptide == "YNALDLTNNGK" && prot_name == allExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides[l]->Protein)
	{
	...
	if(l < allExperiments->ExperimentsList[i]->ProteinsList[j]->Peptides->Count)
	break;
...
void WriteAProteinFile1(ExperimentCollection ^allExperiments, String ^sProtein, List <PeptideHolder ^> ^ProteinPeptides,array<System::String ^> ^args)
{
	...
	//Added by JA 2016.09.30
	//bool bI0I1 = true;

	//Added by JA 2016.10.07
	float rkd, rks, fx1;
	...
	//const char szTempMean = "MeanRates.csv";
	//char *szTempMean = (char *) calloc(13, sizeof(char));
	//
	String ^szTempMean;
	...
	/*
	if(allExperiments->ExperimentsList[0]->ProteinsList[j]->accession == "TRFE_HUMAN")
	{
	printf("Problem Protein");
	}
	*/
	//The following indices can be used to relate the 0th time point to the other time points
	jtemp = -1;
	ltemp = -1;
	...
	for(m = 0; m < ProteinPeptides->Count; m++)
	{
		...
		/*if(ProteinPeptides[m]->Peptide=="ALVDTLK")
		{*/
		//Added by JA 2016.08.19
		FoundPeptide = 0;
		...
		{
		fprintf(fp, "%10.5f, %10.5f\n", lbfgs->fParams[0]/tmax, lbfgs->fParams[1]/tmax);
		}
		else
		{
		fprintf(fp, "%10.5f, %10.5f\n", lbfgs->fParams[1]/tmax, lbfgs->fParams[0]/tmax);
		}
		}
...
void WriteNNLSIsotopeDistribution(ExperimentCollection ^allExperiments, String ^sProtein, List <PeptideHolder ^> ^ProteinPeptides,array<System::String ^> ^args)
{
	...
	{
		/*
		if("AEFAEVSK" == ProteinPeptides[m]->Peptide)
		{
		printf("%s,", ProteinPeptides[m]->Peptide);
		}
		*/
...
void Read_Params(array<System::String ^> ^args)
{
	...
	printf("Params: MassAccuracy = %10.3f PeptideConsistency = %d, ProteinConsistency = %d, PeptideScore = %5.2f PeptideExpectation = %e\n", 
		dMassAccuracy, nPeptideConsistency, nProteinConsistency, dIonScoreCut, dExpectCut);
...
void ProteinRates(array <float> ^fTimeCourse,  array <float> ^fBodyWaterEnrichment)
{
	char szLine[5056], szTemp1[2040], szTemp2[2030], szTemp3[2030], szTemp4[2040];
	...
	*/
	...
	ProtRate = gcnew ProteinRateConstant(CurrentFolder, fTimeCourse, fBodyWaterEnrichment);
...
int Parse_InputTextFile(String ^strFile, array <String ^> ^ mzmlFiles, array <String ^> ^ mzidFiles, array <float> ^fTimePoints, array <float> ^fBodyWaterD2O)
{
	...
	int i, k, j, nExperiments, len, nFilePath;
